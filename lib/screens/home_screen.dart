...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
  String? _path, _name;
  int? _size;
  bool _split = false, _rooted = false, _colorOS = false, _hyperOS = false;
  ...
  Future<void> _init() async {
    ...
    setState(() { _rooted = r; _colorOS = c; _hyperOS = h; _logPath = l; });
  }
  ...
  Future<void> _pick() async {
    ...
    try {
      ...
      if (r?.files.single.path != null) {
        ...
        setState(() {
          _path = p; _name = r.files.single.name; _size = s.size;
          _split = ApkInstallerService.isSplit(p);
          _status = InstallStatus.idle; _msg = '';
        });
  ...
  Future<void> _install(InstallMethod method) async {
    ...
    try {
      ...
      final code = _split
          ? await ApkInstallerService.installSplitApk(_path!)
          : switch (method) {
  ...
  void _reset() {
    setState(() { _path = _name = null; _size = null; _split = false; _status = InstallStatus.idle; _msg = ''; });
    _anim.reverse();
  }
  ...
  void _logDialog() => showDialog(context: context, builder: (ctx) => AlertDialog(
    ...
    content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(padding: const EdgeInsets.all(10),
        ...
        child: Text(_logPath ?? 'Downloads/installer_log.txt',
    ...
    actions: [
      TextButton(
        onPressed: () async {
          await ApkInstallerService.clearLog();
          if (ctx.mounted) { Navigator.pop(ctx); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Log effacé'))); }
        },
      ...
      FilledButton(
        onPressed: () {
          ...
          Navigator.pop(ctx);
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Chemin copié')));
        },
        ...
        children: [
          ...
          Container(
            ...
            child: const Row(children: [
              ...
              SizedBox(width: 10),
              Expanded(child: Text('Installation via com.android.vending',
                style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF1A73E8)))),
              Icon(Icons.verified_rounded, color: Color(0xFF1A73E8), size: 16),
            ]),
          ),

          // Badges
          if (_colorOS || _hyperOS || _rooted || _split) ...[
            const SizedBox(height: 10),
          ...
          AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: _path != null ? cs.primary.withOpacity(0.5) : Colors.grey.withOpacity(0.3),
                width: 2,
              ),
            ),
            child: InkWell(
                  ...
                  child: _path == null
                      ? Column(key: const ValueKey('e'), children: [
                          Container(width: 72, height: 72,
                            ...
                            child: Icon(Icons.folder_open_rounded, size: 38, color: cs.primary)),
                      ...
                      : FadeTransition(key: const ValueKey('f'), opacity: _fade,
                          child: ApkCard(name: _name!, path: _path!, size: _fmt(_size ?? 0))),
                ),
              ),
            ),
          ),
          ...
          if (_path == null)
            ...
            _mainBtn(
              _split ? 'Installer (split)' : 'Installer',
              Icons.system_update_alt_rounded,
              const Color(0xFF1A73E8),
              busy ? null : () => _install(InstallMethod.standard),
              busy,
            ),
            if (!_split) ...[
              ...
              _div(cs),
              const SizedBox(height: 12),
              _altBtn('Oppo / Realme (ColorOS)', 'OppoTrick — bypass scanner ColorOS',
                Icons.phonelink_setup_rounded, Colors.orange,
                busy ? null : () => _install(InstallMethod.oppoNoRoot)),
              const SizedBox(height: 10),
              _altBtn('Xiaomi / Redmi / Poco (HyperOS)', 'Contourne le Security Center',
                Icons.shield_outlined, Colors.deepOrange,
                busy ? null : () => _install(InstallMethod.hyperOS)),
              const SizedBox(height: 10),
              _altBtn('Installer via Root', 'pm install -i com.android.vending',
                Icons.security_rounded, _rooted ? Colors.green : Colors.grey,
                busy ? null : () {
                  if (!_rooted) showDialog(context: context, builder: (ctx) => AlertDialog(
                    title: const Text('Root non détecté'),
                    content: const Text('Cette méthode nécessite root.'),
  ...
  Widget _div(ColorScheme cs) => Row(children: [
    ...
    Padding(padding: const EdgeInsets.symmetric(horizontal: 12),
      child: Text('Méthodes alternatives',
        style: TextStyle(fontSize: 11, color: cs.onSurface.withOpacity(0.4)))),
    Expanded(child: Divider(color: cs.outline.withOpacity(0.3))),
  ]);

  Widget _mainBtn(String l, IconData ic, Color c, VoidCallback? fn, bool loading) => SizedBox(
    height: 56,
    child: FilledButton(
      onPressed: fn,
      ...
      child: loading
          ? const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              SizedBox(width: 10),
              Text('En attente…', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700)),
            ])
          : Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(ic, size: 20), const SizedBox(width: 10),
              Text(l, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
            ]),
    ),
  );

  Widget _altBtn(String l, String sub, IconData ic, Color c, VoidCallback? fn, {bool disabled = false}) => Opacity(
    ...
    child: OutlinedButton(
      ...
      child: Row(children: [
        ...
        Icon(Icons.arrow_forward_ios_rounded, size: 13, color: c.withOpacity(0.5)),
      ]),
    ),
  );
  ...
  Widget _compat(ColorScheme cs) {
    const items = [
      ('Pixel', Icons.smartphone_rounded), ('Samsung', Icons.phone_android_rounded),
      ('OnePlus', Icons.devices_rounded), ('Oppo', Icons.phone_iphone_rounded),
      ('Realme', Icons.phone_rounded), ('Xiaomi', Icons.phone_android_rounded),
      ('LineageOS', Icons.settings_applications_rounded),
    ];
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Wrap(spacing: 8, runSpacing: 8, children: items.map((b) => Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(b.$2, size: 13, color: cs.primary), const SizedBox(width: 4),
          Text(b.$1, style: TextStyle(fontSize: 12, color: cs.onSurface.withOpacity(0.8))),
        ]),
      )).toList()),
    ]);
  }
